...
void editing(AEROFLOT *flight, int countRecords);
...
int menuTitle(){
	...
	std::cout << "10. Edit field" << std::endl;
...
int fromFile( AEROFLOT** flight, int *maxRecord, int *countRecords ){
	...
	(*countRecords) = 0;
	// чтение из файла
	std::ifstream in ("AEROFLOT1.txt", std::ifstream::in);
    in.seekg (-1,in.end);
    long size = in.tellg();
    size = size + 2;
    in.seekg (0, in.beg);
...
void editing(AEROFLOT *flight, int countRecords){
	...
	if ( strcmp( field, "destination") == 0){
			std::cin >> strValue;
			strcpy( flight[num].destination, strValue );


	} else if ( strcmp( field, "planeType" ) == 0 ){
			std::cin >> strValue;
			strcpy( flight[num].planeType, strValue );


	} else if ( strcmp( field, "flightNumber" ) == 0 ){
		std::cin >> digitValue;
		flight[num].flightNumber = digitValue;
